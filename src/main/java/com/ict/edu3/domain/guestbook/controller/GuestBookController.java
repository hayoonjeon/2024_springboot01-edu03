...
public class GuestBookController {
  ...
  public DataVO getGuestBookById(@PathVariable String gb_idx) {
    DataVO dataVO = new DataVO();
    try {
      ...
      if (gvo==null) {
        dataVO.setSuccess(false);
        dataVO.setMessage("GuestBook 상세보기 실패 ");
        return dataVO;
      }
      dataVO.setSuccess(true);
      dataVO.setMessage("GuestBook 상세보기 성공 ");
      dataVO.setData(gvo);
    } catch (Exception e) {
      dataVO.setSuccess(false);
  ...
  public DataVO getGuestBookDelete(@PathVariable String gb_idx,Authentication authentication) {
    ...
    try {
      ...
      int result = 0;
      result = guestBookService.getGuestBookDelete(gb_idx);
      if (result==0) {
        dataVO.setSuccess(false);
        dataVO.setMessage("GuestBook 삭제 실패 ");
        return dataVO;
      }
      dataVO.setSuccess(true);
      dataVO.setMessage("GuestBook 삭제 성공 ");
    } catch (Exception e) {
      dataVO.setSuccess(false);
      dataVO.setMessage("게스트북 삭제 오류발생");
    }
      return dataVO;
  }
